...
class AnkiClient:
    ...
    def create_deck_if_missing(self) -> None:
        ...
        try:
            ...
            print(
                f"[bold yellow]Warning:[/bold yellow] Failed to ensure Anki deck exists: {e}"
            )
    ...
    def add_card(
        ...
        known_words: Optional[str] = None,
        unknown_words: Optional[str] = None,
        ...
        if audio_path:
            ...
            try:
                ...
                print(
                    f"[bold yellow]Warning:[/bold yellow] Failed to upload audio: {e}"
                )
        ...
        if image_path:
            ...
            try:
                ...
                print(
                    f"[bold yellow]Warning:[/bold yellow] Failed to upload image: {e}"
                )

        stats_html = (
            '<br><hr><div style="text-align: left; font-size: 0.8em; color: #888;">'
        )
        if known_words:
        ...
        try:
            ...
            print(
                f"[bold yellow]Warning:[/bold yellow] Failed to format front template: {e}. Falling back to default layout."
            )
        ...
        try:
            ...
            print(
                f"[bold yellow]Warning:[/bold yellow] Failed to format back template: {e}. Falling back to default layout."
            )
        ...
        try:
            ...
            if "duplicate" in str(e).lower():
                return -2
            print(
                f"[bold yellow]Warning:[/bold yellow] Failed to add card to Anki: {e}"
            )
            return None
    ...
    def get_deck_words(self) -> list[str]:
        ...
        try:
            ...
            print(
                f"[bold yellow]Warning:[/bold yellow] Failed to load known words from Anki deck '{self.deck_name}': {e}"
            )
            return []
